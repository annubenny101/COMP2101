...
#assigning hostname,lanname,external ip and external name to desired variables accordingly
hostname=$(hostname)
myipaddress=$(ip a s $( ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
mylanhostname=$(getent hosts $myipaddress | awk '{print $2}')
myExternalIP=$(curl -s icanhazip.com)
myExternalName=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')

#Add variables for the default router's name and IP address.
#here, we get the routers name and address dynamically

drouteraddr=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
droutername=$(getent hosts $drouteraddr | awk '{print $2}')
...
Hostname   :$hostname
LAN Address :$myipaddress
LAN Hostname :$mylanhostname
External IP  :$myExternalIP
External Name :$myExternalName

Router Address : $drouteraddr
Router Hostname : $droutername
